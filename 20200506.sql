...
-- 데이터 검증, tax에 있는 시도, 시군구가 fastfood에 없나 확인
SELECT sido, sigungu
FROM tax
...
크게 다음 두가지 형태로 사용
1. 테이블의 모든 컬럼에 값을 입력하는 경우, 컬렴명을 나열하지 않아도 된다
...
9999번 사번으로 jw 사원을 입력, 입사일자는 2020년 4월 13일로 설정하여 데이터 입력
...
WHERE 절이 없으면 테이블의 모든 행의 부서명과 위치를 변경해버림
...
UPDATE, DELETE : 기존에 있는걸 변경, 삭제
    ==> 쿼리를 작성할 경우 주의
        ...
        2. UPDATE, DELETE 문을 싱해하기 전에 WHERE절을 복사해서 SELECT를 하며
           영향이 가는 행을 눈으로 확인
...
WHERE 절을 사용 가능한 DML : SELECT, UPDATE, DELETE
    3개의 쿼리는 데이터를 식별하는 WHERE 절의 사용 될 수 있다
...
emp_cp 테이블을 TRUNCATE 명령을 통해 모든 데이터를 삭제
...
트랜잭션은 ROLLACK, COMMIT을 통해 종료가 된다
...
두 개의 INSERT 쿼리가 게시글 등록의 트랜잭션 단위
