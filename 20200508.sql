...
dname 컬럼에 설정된 NOT NULL 제약조건에 의해 아래 INSERT 구문이 실패한다
INSERT INTO dept_test VALUES (19, NULL, 'deajeon');

문자열의 경우 ''를 NULL로 인식한다
아래의 INSERT 구문 애러
INSERT INTO dept_test VALUES (19, '', 'deajeon');
...
ex: emp 테이블에 데이터를 입력할 때 deptno 컬럼의 값이 dept 테이블에 존재하는 deptno 갑
    이어야 입력 가능
...
현재 부서 테이블 에는 99번 부서만 존재 
FOREIGN KEY 제약조선에 의해 emp 테이블 deptno 컬럼에는 99번 이외의 부서번호는 입력될 수 없다

99번 부서는 존재하지 않으므로 정상적으로 입력할 수 없다
...
CREATE TABLE emp_test(
 ...
 CONSTRAINT 제약조건 명제약조건 TYPE 컬럼 REFERENCES 참조하는 테이블 (컬럼)
...
(9999, 'brown', 99) ===> (99, 'ddit', 'deajeon')

dept_test 테이블의 99번 부서의 데이터를 지우면 어떻게 될까?
(9999, 'brown', 99) ===> 
;
부모 레코드 (dept_test.deptno)를 참조하고 있는 자식 레코드()가
...
1. defualt ==> 참조하고 있는 부모가삭제 될 수 없다
...
2. 테이블 명세서가 정확하지 않으면 신규 개발자는 해당 내용을 모를 수가 있음
    ...
    신기하게도 emp테이블의 데이터가 삭제되거나 null 로 설정되는 경우를 볼 수 있음
...
emp 테이블에서 급여 정보(sal)를 관리할 때 sal 컬럼의 값은 0 보다 작은 값이 들어가는 것은
로직적으로 이상함
...
이미 생성된 테이블을 수정 할 수도 있음
    ...
    ********* 기존 생성되어 있는 컬럼 중간에는 새로운 컬럼을 추가할 수 없다
            ===> 기존 테이블을 삭제하고 컬럼 순서를 조정한 새로운 테이블 생성 DDL로
                 새롭게 테이블을 만들어야 함
                 운영중이라면 이미 데이터가 쌓여 이을 가능성이 굉장히 높음
                 ==> 테이블을 생성할 때 신중하게 컬럼 순서를 고려
    2. 기존 컬럼 삭제, 컬럼 이름 변경, 컬럼 사이즈 변경, (타입도 제한적으로 변경 가능)
       참조키가 걸려있는 테이블의 컬럼 이름을 변경하더라도 참조하고 있는 테이블에는 영향이 가지 않음
       (알아서 이름 변경 해준다)
...
위에서 추가한 hp 컬럼의 컬럼 사이즈를 20 에서 30 으로 변경
...
위에서 추가한 emp_test 테이블 hp 컬럼을 hp_n으로 이름 변경
...
DDL은 자동 커밋이기 때문에 DML 문자에도 영향을 받는다
